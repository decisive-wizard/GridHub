(function (Handsontable) {
  ...
  function HandsontableFormula() {
    ...
    var formulaRenderer = function (instance, TD, row, col, prop, valueObj, cellProperties) {
      // handsontable related hack, if incorrectly stringifies the valueObj,
      // query handsontable for the non stringified version
      // couldnt figure out where to stop stringifying
      if (typeof valueObj !== 'object'){
        valueObj = instance.getDataAtCell(row,col);
      }
      ...
      if (instance.formulasEnabled) {
        ...
        if (item) {
          ...
          // add item to matrix
          item = instance.plugin.matrix.addItem(item);
        }

        // check if typed formula or cell value should be recalculated
        if ((valueObj.formula && valueObj.formula[0] === '=') || needUpdate) {
          
          if (valueObj.formula) {
            formula = valueObj.formula.length > 0 ? valueObj.formula.toString().substr(1).toUpperCase() : "";
          } else {
            formula = "";
          }
          ...
          if (newValue.result !== null){
            result = typeof newValue.result === 'object' ? newValue.result.value : newValue.result;
            
            // update item value and error
            instance.plugin.matrix.updateItem(item, {formula: formula, value: result, error: newValue.error, needUpdate: false});

            error = newValue.error;

            // update cell in hot
            valueObj.value = error || result;
          }
        }

        if (error) {
          valueObj.value = error;
        }
      ...
      // apply changes
      textCell.renderer.apply(this, [instance, TD, row, col, prop, valueObj, cellProperties]);
    };
    ...
    var afterChange = function (changes, source) {
      ...
      if (source === 'edit' || source === 'undo' || source === 'autofill') {
        ...
        changes.forEach(function (item) {
          ...
          // if ((valueObj.formula && valueObj.formula[0] !== '=') ||
          // (prevValueObj && prevValueObj.formula !== valueObj.formula) ||
          // (prevValueObj && prevValueObj.value !== valueObj.value)) {
            var item = instance.plugin.matrix.getItem(cellId);
            if (item){
              instance.plugin.matrix.updateItem(cellId,{value: valueObj.value });
            } else {
              item = {
                id: cellId,
                value: valueObj.value,
                formula: valueObj.formula
              }
              instance.plugin.matrix.addItem(item);
            }
    ...
    var beforeAutofillInsidePopulate = function (index, direction, data, deltas, iterators, selected) {
      ...
      if (valueObj.formula[0] === '=') { // formula
        ...
        return {
          value: instance.plugin.utils.updateFormula(valueObj.formula, direction, delta),
          iterators: iterators
        }
        ...
        if (rlength >= 2 || clength >= 2) {
          ...
          var newValue = instance.plugin.helper.number(valueObj.value),
              ii,
              start;
